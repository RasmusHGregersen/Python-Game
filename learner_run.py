...
class Q_Learner(object): #from Learner
    ...
    def choose_key(self, snake, food):
        ...
        direction = self.learner_directions_dict[action_key]
    ...
    def UpdateQValues(self, death):
        
        activity_log = self.activity_log[::-1] #reverse
        for state_number,state_data in enumerate(activity_log[:-1]):
            
            if death: # Snake Died -> Negative reward
                
                current_state = activity_log[0]['state']
                current_action = activity_log[0]['action']
                state_str = str((current_state.position[0],current_state.position[1],current_state.relative_state))
    
                reward = -1
                self.q_table[state_str][current_action] = (1-self.learning_rate) * self.q_table[state_str][current_action] + self.learning_rate * reward # Bellman equation - there is no future state since game is over
                reason = None
    ...
    def FutureState(self, snake, food):
        ...
        return State_Class((dist_x, dist_y), (pos_x, pos_y), relative_state, food)
...
class SnakeGame():
    bounds= None
    human_player = False #had to change here
    timed= False
    speed= None
    pixel_size=20
    game_over_font=None
